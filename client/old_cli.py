...
# todo rewrite cli -> handle state better
    ...
    def create_new_playlist(self, options: list) -> int:
        ...
        if len(options) == 2:
            ...
            if isinstance(options[0], int):
                name = self.ask_string("enter playlist name")
                bpm = options[0]
            else:
                name = options[0]
                bpm = self.ask_int("enter bpm")
        elif len(options) == 0:
            name = self.ask_string("enter playlist name")
            bpm = self.ask_int("enter bpm")
        else:
        ...
        self.api_instance.get_playlist_daw_get()
        return 0
...
def serve(host: str = "http://127.0.0.1:8001", port: int = 8001):
    uvicorn.run(app,
                # host=host,
                # port=port,
                # log_level="critical",
