...
changes colors after every 44100 samples to list_commands_func visually break up processing
times.
...
Complete list_commands_func for MrsWatson can be found by running the program with no
arguments, or with `--list_commands_func`. To see extended list_commands_func for all options (which prints
quite a lot of output) run `mrswatson --list_commands_func full`.
...
on presets, run `mrswatson --list_commands_func plugin`.
...
may list_commands_func to solve the problem.
